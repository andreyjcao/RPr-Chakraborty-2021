...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Andrey Cao"
date: "`r Sys.Date()`"
...
filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry() %>% 
  t() %>%
  kable(align = "c", caption = "Missing Data") %>%
...
## Map race and ethnicity rates

NEW MODIFICATION: We can also map the spatial distributions of the percent of black people with disability and the percent of non-Hispanic non-White people with disability (the two variables with the strongest positive Pearson's r (Table 1 of original study)). Mapping these two variables can help us to understand factors driving results in the GEE models such as the more diverse demographics in the South and Southwest. This is important to the study, as those regions also showcase greater COVID spread. 

```{r map-black-rates NEW, message = FALSE}
tm_black_rates <- tm_shape(acs_covid) +
  tm_polygons("black_pct",
    title = "Percent of Black People with Disability\nof Total Population\n(ACS 2014-2018)",
    style = "quantile",
    border.alpha = .2,
    lwd = 0.2,
    palette = "Oranges"
...
```{r map-nonhispnonw-rates NEW, message = FALSE}
tm_nonhispnonw_rates <- tm_shape(acs_covid) +
  tm_polygons("non_hisp_non_white_pct",
    title = "Percent of Non-Hispanic Non-White People\nwith Disability of Total Population\n(ACS 2014-2018)",
    style = "quantile",
    border.alpha = .2,
    lwd = 0.2,
    palette = "Oranges"
...
tm_gee_weights <- tm_shape(gee_resid) +
  tm_polygons("w",
    ...
    lwd = 0.2,
    palette = "Oranges",
    legend.hist = FALSE #deleted midpoint = 0 to utilize full color ramp
  ) +
  tm_shape(state) +
    tm_borders("grey", lwd = .5) +
  tm_layout(
...
## Deviations, Fixes, and Checks for Reproducibility
...
1. vector layer of circle polygons with the center and radius of each cluster, ID of the county at the center of the cluster, and a relative risk score for the cluster.
...
We also created maps visualizing the spatial clusters of COVID-19 incidence based on the output of SpatialEpi and SaTScan.

We then discovered that the original study most likely operationalized COVID-19 risk as the local relative risk of the county at the center of the cluster using the vector layer of circle polygons enumerated as the second output above (Figure 4).
...
We then observed the spatial heterogeneity of local relative risk (Figure 5) and considered the original purpose of calculating clusters and relative risk classes, which was to control for spatial dependence within states and COVID-19 hotspots.
...
Lastly, we created a dot-and-whisker plot of coefficients for each of the five model variations to compare coefficients across each model, starting with the results published in the original study.
The next two models make incremental adjustments to the research design and are most compatible with the goals of a reproduction study.

Some coefficients remained significant and robust to changes in computational environment, data errors, and clustering criteria.
Robustly positive coefficients included Black, Native American, Non-Hispanic non-White, Below poverty level, and Female, while Asian and Age 5-17 coefficientsremained just above 0.
All of these categories of people with disabilities would be considered intersectional with factors of vulnerability.
Conversely, some robustly negative coefficients indicate people with disabilities intersecting with non-vulnerable identities, such as white, Non-Hispanic White, Above poverty level, and Male.
Counter-intuitively, the two coefficients for elderly people with disability were also robustly negative.
...
This version was made with a small amount of modifications to the second version .
...
Modifications to the second version of our reproduction report focused on improving cohesiveness and clarity of the discussion and conclusion sections, small changes to figures and tables, and the addition of maps for clarification on relative risk models.

To improve the discussion and conclusion sections, we adjusted the position of the "rationale for the updated report," integrated Emily's discussion and new results, and separated the conclusions into their own section. 
These changes improved the structure of the report and gave those producing future reproductions of this study a more clearer understanding of the results and significance of the study. 
Additionally, the inclusion of new results into the discussion question provided the most updated information for future reproductions of this study.

The minor changes to figures included adjustments to the final choropleth map as well as improving formatting of the 'missing data' table. 
Both of these changes improved the readability of the report by creating a more visually understandable map figure and formatting a table for html view.

The addition of two maps (one of percentages of black people with disability, one of percentages of non-Hispanic non-White people with disability) helped clarify why some factors/conditions are driving results in the GEE models, such as the more diverse demographics in the South and Southwest
